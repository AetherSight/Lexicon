"""
FFXIV equipment label system definitions
Defines various attribute labels for equipment image annotation
"""
...
4. **风格**：{styles}
5. **特殊效果**：{effects}
6. **外观类型**：{appearance_types}（可以选择多个，例如：["双手剑", "盾牌"]）
7. **自定义标签**：必须提供至少5个、最多10个自定义标签。这些标签是你经过思考后认为应该添加但上面1-7类中没有的标签，用于描述装备的独特特征。每个标签不得超过8个汉字。
